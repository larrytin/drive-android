...
public class HarmonyActivity extends BaseActivity implements OnClickListener {
  ...
  private class ResultAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
      ...
      if (title.matches("^\\d{4}.*")) {
        ...
        holder.common_result_yellow.setText(title);
      }
      holder.common_result_yellow.setOnClickListener(new OnClickListener() {
        @Override
        public void onClick(View v) {
          ...
          JsonArray tags =
              Json.createArray().push(Constant.DATAREGISTRY_TYPE_HARMONY).push(currentGrade).push(
                  currentTerm).push(currenTopic).push(title);
  ...
  private class ResultAdapterHolder {
    private TextView common_result_yellow;
  }
  ...
  private void onPageSelected(int position) {
    ...
    ll_act_harmony_result_bar.removeAllViews();
    for (int i = 0; i < totalPageNum; i++) {
      ImageView imageView = new ImageView(HarmonyActivity.this);
      ...
      ll_act_harmony_result_bar.addView(imageView);
    }
  }
  ...
  private void sendQueryMessage(JsonArray tags) {
    ...
    bus.sendLocal(Constant.ADDR_TAG_CHILDREN, msg, new MessageHandler<JsonObject>() {
      ...
      public void handle(Message<JsonObject> message) {
        ...
        JsonArray attachments = body.getArray(Constant.TAGS);
