...
public class VideoActivity extends BaseActivity {
		...
		public void handleMessage(Message msg) {
			...
			case PROGRESS_CHANGED:// 进度改变
				...
				if (isOnline) {
					...
					sb_media_controler_seekbar.setSecondaryProgress(j
							* sb_media_controler_seekbar.getMax() / 100);
				} else {
				...
				int second = i % 60;
				minute %= 60;
				tv_media_controler_has_played.setText(String.format(
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		this.controlView = getLayoutInflater().inflate(
				R.layout.video_media_controler, null);
		this.controlerWindow = new PopupWindow(this.controlView);
		this.tv_media_controler_duration = (TextView) this.controlView
				.findViewById(R.id.tv_media_controler_duration);
		this.tv_media_controler_has_played = (TextView) this.controlView
				.findViewById(R.id.tv_media_controler_has_played);

		this.titleView = getLayoutInflater().inflate(
				R.layout.video_media_title, null);
		this.titleWindow = new PopupWindow(this.titleView, this.screenWidth, 60);

		Button bt_media_title_back = (Button) titleView
				.findViewById(R.id.bt_media_title_back);
		Button bt_media_title_screen = (Button) titleView
				.findViewById(R.id.bt_media_title_screen);

		this.videoView = (VideoView) findViewById(R.id.vv);

		this.ibtn_media_controler_play_pause = (ImageButton) this.controlView
				.findViewById(R.id.ibtn_media_controler_play_pause);
		this.btn_media_controler_sound = (Button) this.controlView
				.findViewById(R.id.btn_media_controler_sound);
		this.btn_media_controler_brightness = (Button) this.controlView
				.findViewById(R.id.btn_media_controler_brightness);
		...
		Looper.myQueue().addIdleHandler(new IdleHandler() {
			...
			public boolean queueIdle() {// 空闲的队列
				...
				if (controlerWindow != null && videoView.isShown()) {
					controlerWindow.showAtLocation(videoView, Gravity.BOTTOM,
							0, 0);
		...
		this.videoView.setOnPreparedListener(new OnPreparedListener() {// 注册在媒体文件加载完毕，可以播放时调用的回调函数
					...
					public void onPrepared(MediaPlayer arg0) {// 加载
						...
						int second = i % 60;
						minute %= 60;
						tv_media_controler_duration.setText(String.format(
								"%02d:%02d:%02d", hour, minute, second));

						videoView.start();
						ibtn_media_controler_play_pause
								.setImageResource(R.drawable.video_selector_player_play);
						hideControllerDelay();
		...
		this.btn_media_controler_sound
				.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						getPopupVolumn();
						popupVolumn.showAtLocation(findViewById(R.id.vv),
								Gravity.CENTER, 0, 0);
					}
				});
		...
		this.btn_media_controler_brightness
				.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						getPopupBrightness();
						popupBrightness.showAtLocation(findViewById(R.id.vv),
								Gravity.CENTER, 0, 0);
					}
				});

		this.sb_media_controler_seekbar = (SeekBar) controlView
				.findViewById(R.id.sb_media_controler_seekbar);
		this.sb_media_controler_seekbar
				.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
					...
					public void onStopTrackingTouch(SeekBar seekBar) {
						subHandler
								.sendEmptyMessageDelayed(HIDE_CONTROLER, TIME);
					}
				});
		...
		if (uri != null) {
			...
			this.ibtn_media_controler_play_pause
					.setImageResource(R.drawable.video_selector_player_pause);
			((TextView) this.titleView.findViewById(R.id.tv_media_title_name))
					.setText("我叫大白菜");
		}
	}
	...
	protected void initPopuptBrightness() {
		...
		popupBrightness = new PopupWindow(popupWindow_view,
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, true);// 创建PopupWindow实例

		// 注意要加这句代码，点击弹出窗口其它区域才会让窗口消失
		popupBrightness.setBackgroundDrawable(new ColorDrawable(0x00000000));

		brightnessPercent = (TextView) popupWindow_view
				.findViewById(R.id.brightness_controler_percent_text);
		brightnessSeekbar = (SeekBar) popupWindow_view
				.findViewById(R.id.brightness_controler_seekbar);
	...
	protected void initPopuptVolumn() {
		...
		popupVolumn = new PopupWindow(popupWindow_view,
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, true);// 创建PopupWindow实例

		// 注意要加这句代码，点击弹出窗口其它区域才会让窗口消失
		popupVolumn.setBackgroundDrawable(new ColorDrawable(0x00000000));

		volumePercent = (TextView) popupWindow_view
				.findViewById(R.id.volume_controler_percent_text);
		volumeSeekbar = (SeekBar) popupWindow_view
				.findViewById(R.id.volume_controler_seekbar);
