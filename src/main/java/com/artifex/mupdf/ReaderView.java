...
public class ReaderView extends AdapterView<Adapter> implements GestureDetector.OnGestureListener,
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    if (mCurrent > 0) {
      ...
      lv.layout(cvLeft - lv.getMeasuredWidth() - gap,
          (cvBottom + cvTop - lv.getMeasuredHeight()) / 2, cvLeft - gap, (cvBottom + cvTop + lv
              .getMeasuredHeight()) / 2);
    }
    ...
    if (mCurrent + 1 < mAdapter.getCount()) {
      ...
      rv.layout(cvRight + gap, (cvBottom + cvTop - rv.getMeasuredHeight()) / 2, cvRight
          + rv.getMeasuredWidth() + gap, (cvBottom + cvTop + rv.getMeasuredHeight()) / 2);
    }
  ...
  private Point getCorrection(Rect bounds) {
    return new Point(Math.min(Math.max(0, bounds.left), bounds.right), Math.min(Math.max(0,
        bounds.top), bounds.bottom));
  }
  ...
  private Rect getScrollBounds(View v) {
    ...
    return getScrollBounds(v.getLeft() + mXScroll, v.getTop() + mYScroll, v.getLeft()
        + v.getMeasuredWidth() + mXScroll, v.getTop() + v.getMeasuredHeight() + mYScroll);
  }
  ...
  private void measureView(View v) {
    ...
    float scale =
        Math.min((float) getWidth() / (float) v.getMeasuredWidth(), (float) getHeight()
  ...
  private void slideViewOntoScreen(View v) {
    ...
    if (corr.x != 0 || corr.y != 0) {
      mScrollerLastX = mScrollerLastY = 0;
      // mScroller.startScroll(0, 0, 0, corr.y, 400);
